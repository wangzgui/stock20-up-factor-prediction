"""
V3.2-A1.1 pre-registered fusion diagnostics (no submission).
...
Reports:
  ...
  future-market regimes, and fixed fusion-rule comparisons.
"""
...
FUSION_VETO_BOTTOM_PCT = 0.20
FUSION_TEST_POOL_SIZE = 50
FUSION_TEST_UNIVERSE_SIZE = 1500
FUSION_POOL_FRACTION = FUSION_TEST_POOL_SIZE / FUSION_TEST_UNIVERSE_SIZE
...
def print_prediction_decile_diagnostic(
    ...
    print("\nDIAGNOSTIC 1/5: predicted relative-score decile monotonicity")
...
def print_topk_diagnostic(
    ...
    print("\nDIAGNOSTIC 2/5: portfolio performance at different K")
    print(table.to_string(index=False, formatters={
        "base_ret": "{:.3%}".format,
        "A11_ret": "{:.3%}".format,
        "ret_delta": "{:+.3%}".format,
...
def print_top5_replacement_diagnostic(
    ...
    print("\nDIAGNOSTIC 3/5: exact Top5 replacements caused by A1.1")
    print(
...
def print_future_market_regime_diagnostic(
    ...
    table = pd.DataFrame(summaries)
    print("\nDIAGNOSTIC 4/5: Top5 performance by future market regime")
    print(
...
def _top_by_score(score, base_probability, k=5):
    """Descending score with base probability as a deterministic tie-break."""
    order = np.lexsort((-base_probability, -score))
    return order[:min(k, len(order))].astype(np.int32)


def fixed_fusion_selections(base_probability, relative_prediction):
    """Return pre-registered Top5 selections; no threshold/grid search."""
    n = len(base_probability)
    base_rank = percentile_rank(base_probability)
    relative_rank = percentile_rank(relative_prediction)
    base_order = np.argsort(-base_probability, kind="stable")
    baseline = base_order[:5].astype(np.int32)

    # Rule 1: relative score can veto only the clearly weak bottom 20%; it can
    # never promote a lower-base stock ahead of another eligible base stock.
    eligible = relative_rank >= FUSION_VETO_BOTTOM_PCT
    veto_order = base_order[eligible[base_order]]
    if len(veto_order) < 5:
        missing = base_order[~np.isin(base_order, veto_order)]
        veto_order = np.r_[veto_order, missing]
    veto = veto_order[:5].astype(np.int32)

    # Rule 2: scale the official-test Top50 pool (50/1500) to each validation
    # cross-section, then let the unchanged base probability make the final cut.
    pool_size = max(5, int(np.ceil(n * FUSION_POOL_FRACTION)))
    relative_pool = np.argsort(-relative_rank, kind="stable")[:pool_size]
    pool_order = relative_pool[
        np.argsort(-base_probability[relative_pool], kind="stable")
    ]
    pool_then_base = pool_order[:5].astype(np.int32)

    # Rule 3a/3b: both ranks must agree. Product is smooth; minimum is the
    # stricter weakest-link intersection. Neither score is treated as a probability.
    rank_product = _top_by_score(base_rank * relative_rank, base_probability, 5)
    rank_minimum = _top_by_score(
        np.minimum(base_rank, relative_rank), base_probability, 5
    )
    return {
        "baseline": baseline,
        "veto_bottom20": veto,
        "relative_pool50": pool_then_base,
...
def score_fixed_selection(y, paths, base_probability, selected):
    """Isolate ranking: chosen assets change, base probabilities/Brier do not."""
    selected = np.asarray(selected, dtype=np.int32)
    selected_p = base_probability[selected]
    if selected_p.sum() > 0:
        weights = selected_p / selected_p.sum()
    else:
        weights = np.full(len(selected), 1.0 / len(selected))
    nav = 0.999 * (weights @ (1.0 + paths[selected]))
    nav[-1] *= 0.999
    full_nav = np.r_[1.0, nav]
    peak = np.maximum.accumulate(full_nav)
    mdd = float(np.max(1.0 - full_nav / peak))
    total_return = float(nav[-1] - 1.0)
    brier = brier_score(y, base_probability)
    score = 0.6 * total_return - 0.2 * mdd - 0.2 * brier
    return score, total_return, mdd, brier


def print_fixed_fusion_diagnostic(
    batches, days, y, relative_y, paths, absolute_tail_y,
    p_up, absolute_prediction, relative_prediction
):
    ret20 = paths[:, -1]
    ...
    for idx in batches:
        base_probability, _, _, _ = batch_probabilities(
            idx, p_up, absolute_prediction, relative_prediction
        )
        selections, aux = fixed_fusion_selections(
            base_probability, relative_prediction[idx]
        )
        baseline_set = set(selections["baseline"])
        market_median = float(np.median(ret20[idx]))

        for rule, selected in selections.items():
            score, total_return, mdd, brier = score_fixed_selection(
                y[idx], paths[idx], base_probability, selected
            )
            selected_set = set(selected)
            removed = np.asarray(sorted(baseline_set - selected_set), dtype=np.int32)
            added = np.asarray(sorted(selected_set - baseline_set), dtype=np.int32)
            day_rows.append({
                "day": int(days[idx[0]]),
                "rule": rule,
                ...
                "mdd": mdd,
                "brier": brier,
                "overlap": len(baseline_set & selected_set) / 5.0,
                "replacements": len(added),
                "pool_size": aux["pool_size"],
            })
            if rule == "baseline":
                continue
            for change, positions in (("removed", removed), ("added", added)):
                for pos in positions:
                    replacement_rows.append({
                        "day": int(days[idx[0]]),
                        "rule": rule,
                        "change": change,
                        "future_return": float(ret20[idx][pos]),
                        "up": int(y[idx][pos]),
                        "absolute_q80": int(absolute_tail_y[idx][pos]),
                        "relative_top20": int(relative_y[idx][pos]),
                        "base_rank": float(aux["base_rank"][pos]),
                        "relative_rank": float(aux["relative_rank"][pos]),
                    })

    frame = pd.DataFrame(day_rows)
    replacement = pd.DataFrame(replacement_rows)
    base = frame[frame["rule"] == "baseline"].set_index("day")
    summaries = []
    rule_order = [
        "baseline", "veto_bottom20", "relative_pool50",
        "rank_product", "rank_minimum",
    ]
    for rule in rule_order:
        g = frame[frame["rule"] == rule].set_index("day").loc[base.index]
        score_delta = g["score"] - base["score"]
        return_delta = g["return"] - base["return"]
        summaries.append({
            "rule": rule,
            "ret": g["return"].mean(),
            "ret_delta": return_delta.mean(),
            ...
            "paired_q20": score_delta.quantile(0.20),
            "win_rate": np.mean(score_delta > 0),
            "overlap": g["overlap"].mean(),
            "replacements": g["replacements"].mean(),
        })
    summary = pd.DataFrame(summaries)

    replacement_summary = []
    if len(replacement):
        for (rule, change), g in replacement.groupby(["rule", "change"], sort=False):
            replacement_summary.append({
                "rule": rule,
                "change": change,
                "n": len(g),
                "future_ret": g["future_return"].mean(),
                "up_rate": g["up"].mean(),
                "absolute_q80_rate": g["absolute_q80"].mean(),
                "relative_top20_rate": g["relative_top20"].mean(),
                "base_rank": g["base_rank"].mean(),
                "relative_rank": g["relative_rank"].mean(),
            })
    replacement_summary = pd.DataFrame(replacement_summary)

    q33, q67 = np.quantile(base["market_median"], [1 / 3, 2 / 3])
    regime_rows = []
    for rule in rule_order[1:]:
        g = frame[frame["rule"] == rule].set_index("day").loc[base.index]
        for regime, mask in (
            ("bear", base["market_median"] <= q33),
            ("neutral", (base["market_median"] > q33) & (base["market_median"] <= q67)),
            ("bull", base["market_median"] > q67),
        ):
            delta = g.loc[mask, "score"] - base.loc[mask, "score"]
            ret_delta = g.loc[mask, "return"] - base.loc[mask, "return"]
            regime_rows.append({
                "rule": rule,
                "regime": regime,
                "days": int(mask.sum()),
    ...
    print("\nDIAGNOSTIC 5/5: pre-registered fusion-rule comparison")
    print(
        f"  veto threshold=bottom {FUSION_VETO_BOTTOM_PCT:.0%}; "
        f"relative pool fraction={FUSION_POOL_FRACTION:.3%} "
        f"(official-test equivalent Top{FUSION_TEST_POOL_SIZE})"
    )
    print("  Selection changes only; weights and Brier use unchanged base probabilities.")
    print(summary.to_string(index=False, formatters={
        ...
        "score": "{:.6f}".format,
        "score_delta": "{:+.6f}".format,
        "median_delta": "{:+.6f}".format,
        "paired_q20": "{:+.6f}".format,
        "win_rate": "{:.3f}".format,
        "overlap": "{:.3f}".format,
        "replacements": "{:.3f}".format,
    }))
    if len(replacement_summary):
        print("\n  Added/removed asset profile by fusion rule:")
        print(replacement_summary.to_string(index=False, formatters={
            "future_ret": "{:.3%}".format,
            "up_rate": "{:.3f}".format,
            "absolute_q80_rate": "{:.3f}".format,
            "relative_top20_rate": "{:.3f}".format,
            "base_rank": "{:.3f}".format,
            "relative_rank": "{:.3f}".format,
        }))
    print("\n  Paired score deltas by future market regime (explanatory only):")
    print(regime_table.to_string(index=False, formatters={
        "ret_delta": "{:+.3%}".format,
        "score_delta": "{:+.6f}".format,
        "win_rate": "{:.3f}".format,
    }))
    return frame, summary, replacement, replacement_summary, regime_table
